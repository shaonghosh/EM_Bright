...
class genDiskMass:
    ...
    def fromMassAmbiguity(self, spin1z=0.):
        '''
        This method uses the samples from the 2D ambiguity ellipse read from an xml 
        file generated by running the standard rapidpe_compute_intrinsic_fisher.py 
        script. The chi1 value is chosen to be zero.
        '''
        masses = readMassXML.readMasses(self.inputFile)
        self.mPrimary = masses[:,0]
        self.mSecondary = masses[:,1]
        self.eta = self.mPrimary * self.mSecondary/((self.mPrimary + self.mSecondary)**2)
        self.chi = np.ones(len(masses))*spin1z
        NS_prob_2 = np.sum(self.mSecondary < 3.0)*100./len(self.mSecondary)
        NS_prob_1 = np.sum(self.mPrimary < 3.0)*100./len(self.mPrimary)
        return [NS_prob_1, NS_prob_2, self.computeRemMass()]
